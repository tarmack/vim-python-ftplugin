...
class TestTypeInferenceEngine(object):
  ...
  def test_iterator(self, tree=None):
    ''' Check if all items that got yielded by Node.walk() are only yielded once.'''
    if tree is None:
      tree = self.tie.tree
    for node in tree:
      assert isinstance(node, typesystem.Node), '%s\'s %s %r is not a Node object.' % (type(tree), type(node), node)
    ...
    for node in ast.walk(ast.parse(source)):
      if id(type(node)) in typesystem.type_mapping:
        ...
        if line is not None and col is not None:
          for tie_node in self.tie.tree.walk():
